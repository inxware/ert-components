...
EHS_FB_RUN_FUNCTION(key_value_upsert)
{
	...
	_err_code = findKeyValue(
		array, item_size, content, key == NULL ? INX_FB_key_value_key : key, 
	...
	switch (_err_code)
	{
		case 0:
		{
			...
			buf_to_write = (char *) EhsHMem_tempAlloc(
				...
				EhsStrlen(value == NULL ? INX_FB_key_value_default_value : value)
			);
			upsertKeyValuePair(
		...
		case 4:
		{
			...
			if (item_size == 1)
			{
				buf_to_write = (char *) EhsHMem_tempAlloc(
					...
					EhsStrlen(value == NULL ? INX_FB_key_value_default_value : value) + 
					EhsStrlen(key == NULL ? INX_FB_key_value_key : key)
				);
				upsertKeyValuePair(
					content, array, key_index, value_index, 
					key != NULL ? key : INX_FB_key_value_key, 
			...
			if (item_size == 0)
			{
				...
				EhsSprintf(
					...
					value == NULL ? INX_FB_key_value_default_value : value
				);
				break;
			}
...
EHS_FB_RUN_FUNCTION(key_value_query)
{
	...
	if (fp == NULL)
	{
		...
		if (EHS_FB_OUT_CONNECTED_API2(INX_key_value_ARG_query_value_out))
			EhsStrcpy(
				EHS_FB_OUT_S_API2(INX_key_value_ARG_query_value_out),
				inx_key_value_state->default_value == NULL ? INX_FB_key_value_default_value : inx_key_value_state->default_value
			) ;
        if (EHS_FB_OUT_CONNECTED_API2(INX_key_value_ARG_query_query_errno))
            EHS_FB_OUT_I_API2(INX_key_value_ARG_query_query_errno) = -1;
	...
	// Find the key
	_err_code = findKeyValue(
		array, item_size, content, inx_key_value_state->key, 
...
function_end:
	...
	if (no_output == EHS_FALSE && EHS_FB_OUT_CONNECTED_API2(INX_key_value_ARG_query_value_out))
		EhsStrcpy(
			EHS_FB_OUT_S_API2(INX_key_value_ARG_query_value_out),
			value != NULL ? value : inx_key_value_state->default_value == NULL ? INX_FB_key_value_default_value : inx_key_value_state->default_value
		) ;
	if (value != NULL)
	{
