...
EHS_FB_FUNCTION_ENTRY("reset", JSONObject_reset)
EHS_FB_FUNCTION_ENTRY("read", JSONObject_read)
EHS_FB_FUNCTION_ENTRY("query", JSONObject_query)
...
static int callback_map_key(void * ctx,const unsigned char * stringVal,size_t stringLen)
{
	...
	if(state->returnLevel > -1 && state->parseLevel > state->returnLevel){
        ...
        //printf("EEEEEEEEEEEEE appending [%s]\n",stringVal);
		if (!state->fieldNameMode) {
            //printf("EEEEEEEEEEEEE field[%s]\n",mapName);
			needcomma(state);
			append(state,"\"");
        ...
        //printf("EEEEEEEEEEEEE Dright level \n");
		...
		if(EhsStrcmp(state->queryPos->map,"?") == 0){
            // printf("EEEEEEEEEEEEE Doing wild-card \n");
			append(state,mapName);
			append(state,",");
            ...
            //printf("EEEEEEEEEEEEE comparing [%s] with [%s]\n",state->queryPos->map,mapName);
            if(EhsStrcmp(state->queryPos->map,mapName)==0){
                //printf("EEEEEEEEEEEEE match [%s]\n",mapName);
...
static ehs_bool testArray(inx_JSONObject_state_type *state){
	...
	//printf("!!!testArray [%x] query levels=[%d=%d]\n",state->arrayPos,state->parseLevel,state->queryLevel);

	if(state->arrayPos!=NULL && state->arrayPos->depth == state->parseLevel){
		//printf("!!!testArray index[%d] query levels=[%d=%d]\n",state->arrayPos->index,state->parseLevel,state->queryLevel);
...
static ehs_bool doQuery(inx_JSONObject_state_type * state , ehs_char * queryString, ehs_char * out) {
	...
	while(input!=NULL){
		...
		status=yajl_parse(state->hand,chunk,size);
		if(status != yajl_status_ok && status != yajl_status_client_canceled){
			unsigned char * str = yajl_get_error(state->hand,1,chunk,size);
			ret = EHS_FALSE;
			EHSH_LOG_WARNING("%s",(const char *)str);
			yajl_free_error(state->hand,str);
