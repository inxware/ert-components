...
EHS_FB_FUNCTION_ENTRY("eos", xml_stream_parser_eos)
EHS_FB_FUNCTION_ENTRY("read", xml_stream_parser_read)
EHS_FB_FUNCTION_ENTRY("parse", xml_stream_parser_parse)
EHS_FB_FUNCTION_ENTRY("streamFinished", xml_stream_parser_streamFinished)
...
EHS_FB_INIT_FUNCTION(xml_stream_parser)
{
	...
	EhsSscanf(EHS_FB_INIT_PARAMETERS,"%hhd %s",&ignore_script,&ignore_entities);
	//printf("ignore_entities=XX%sXX\n",ignore_entities);
...
inx_xml_stream_parser_character_data(void *data,const XML_Char *s,int len){
	...
	printf("inx_xml_stream_parser_character_data[size=%d] = %s",len,(char*) s);
	...
	if(state->cdataString!=NULL){
		...
		//printf("229:cdata:%s",EHS_FB_OUT_S_API2(INX_xml_stream_parser_ARG_parse_cdata));
		//hang on to the new character data
		state->cdataString=s;
		state->cdataLength=len;
		printf("RETURNING FROM CDATA Section in inx_xml_stream_parser_character_data");
		//wait till we are called again
		return;
	}
	printf("Normal data in nx_xml_stream_parser_character_data");
	...
	//printf("======XX CharData 2 \n ");
	EHS_FB_FINISH(INX_xml_stream_parser_ARG_parse_character_data);
	//printf("242:cdata:%s",EHS_FB_OUT_S_API2(INX_xml_stream_parser_ARG_parse_cdata));
}
...
inx_xml_stream_parser_processing(void *data,const XML_Char *target,const XML_Char *instruction){
	...
	//printf("====== XX Processing Function found \n ");
...
static void inx_xml_stream_parser_script_eater_parse(inx_xml_stream_parser_script_eater_type *eater){
	...
	while(1){
		...
		if(eater->state==0){
			//use strstr to find <
			//printf("script end looking from XXX%sXXX\n",eater->position);
			...
			while(1){
				...
				if(script[eater->scriptpos]==c){
					...
					if(eater->scriptpos>=9){
						...
						eater->state=2;
						//printf("341:found end scriptXX%sXX",eater->currentchunk);
						return;
					}
				}else{
...
static void inx_xml_stream_parser_script_eater_start_parse(inx_xml_stream_parser_state_type *state){
	...
	while(1){
		...
		//if state==1 then compare next characters till we get a full match or any mis match
		if(eater->state==1){
			//printf("firstchunkpos=%x position=%x\n",eater->firstchunkpos,eater->position);
			//loop till end of string or match
			c=*eater->position;
			while(1){
				if(c=='\0'){
				...
				if(script[eater->scriptpos]==c){
					...
					if(eater->scriptpos>=7){
						//yep found a match so go to matched state
						eater->state=2;
						//printf("firstchunkpos=%s position=%s\n",eater->firstchunkpos,eater->position);
						return;
					}
				}else{
					//they don't match so reset state
...
static void inx_xml_stream_parser_eat_script(inx_xml_stream_parser_state_type* state){
	...
	//printf("575:start of list is XX%sXX\n",state->input->chunk);
	while ( input != NULL ) {
		...
		if(status==0){
			...
			if(eater->currentchunk==eater->headchunk){
				...
				//printf("XXXXXXXXXXXXXBefore cleaning:%s\nXXXXXXXX",eater->currentchunk->chunk);
				//printf("currentchunk=%x position=%x\n",eater->currentchunk->chunk,eater->position);
				//EhsStrcpy(eater->firstchunkpos,eater->position);
				//printf("firstchunkpos=%x position=%x\n",eater->firstchunkpos,eater->position);
				for(i=0;i<=EhsStrlen(eater->position);i++){
					eater->firstchunkpos[i]=eater->position[i];
				}
				//printf("XXXXXXSame Made:%sxxxxxxx\n",eater->currentchunk->chunk);
			}else{
				//copy trailing first chunk in to place
				eater->firstchunkpos[0]='\0';
				//printf("XXXXXXXXXXXXFirst chunk=%sXXXXXXXXXXXXX\n",eater->headchunk->chunk);
				...
				//printf("Different Made:%sxxxxxxx\n",eater->currentchunk->chunk);
				if(eater->headchunk->pNext!=eater->currentchunk){
					//printf("XXXXXXXXXXXXXsomething went wrongXXXXXXXXXXX\n");
					//printf("XX%sXX%sXX\n",eater->headchunk->pNext->chunk,eater->currentchunk->chunk);
				}
				//now free from the after the start to our current chunk
				input=state->input->pNext;
				while(input!=NULL&&input!=eater->currentchunk){
					state->input->pNext=input->pNext;
					//printf("656:freeingXX%sXX\n",input->chunk);
					EhsHMem_tempFree(input);
					input=state->input->pNext;
				}
				//double check we did this right
				//printf("start:XX%sXX\nnext:XX%sXX\n",state->input->chunk,input->chunk);
			}
...
static void inx_xml_stream_parser_parse(inx_xml_stream_parser_state_type* state){
	...
	if(input==NULL||hand==NULL||pFIdata==NULL){
		printf("parse:No input or handle or pFIdata\n");
		return;
	}
	printf("All Good in inx_xml_stream_parser_parse()");
	input=state->input;
	...
	while( shouldParse == EHS_TRUE ) {
		...
		if(state->pausedForEvent == EHS_TRUE){
			...
			if(state->ignoreScript==EHS_TRUE){
				...
				while(input!=NULL){
					...
					//printf("652:start script eater status=%d\n",status);
					if(status==0){
		...
		if(parsed==EHS_TRUE){
			...
			if(status == XML_STATUS_ERROR) {
				//printf("XML parser error code:%d\n",XML_GetErrorCode(hand));
				//printf("===== XX Error \n");
				EHS_FB_FINISH(INX_xml_stream_parser_ARG_parse_error);
				//printf("XML parser error = %s at line %d\n",XML_ErrorString(XML_GetErrorCode(hand)),XML_GetCurrentLineNumber(hand));
				//printf("|||%s|||\n",input->chunk[XML_GetCurrentColumnNumber(hand)]);
				/*fclose(state->received);
				fclose(state->parsed);*/
				break;
			}else if(status==2){
				//parsing paused due to event
				//printf("paused due to event\n");
				state->pausedForEvent=EHS_TRUE;
				break;
			}else{
				if (state->attrs != NULL) printf("++++++++++++++ attribs not null +++++++++++++\n");
				...
				if(input->pNext!=NULL){
					...
					pOld=input;
					input=pOld->pNext;
					//printf("778:freeingXX%sXX\n",pOld->chunk);
					//printf("826:head isXX%sXX\n",state->input->chunk);
...
EHS_FB_RUN_FUNCTION(xml_stream_parser_read)
{
	...
	if (EHS_FB_IN_CONNECTED_API2(INX_xml_stream_parser_ARG_read_data)){
		...
		//printf("Always doinf ampersand replacement - ignoring user setting due to problem reading parameters.\n");
