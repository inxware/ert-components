...
EHS_FB_FUNCTION_ENTRY("hit", GUI_keypress_hit)
EHS_FB_FUNCTION_ENTRY("enable", GUI_keypress_Enable)
EHS_FB_FUNCTION_ENTRY("disable", GUI_keypress_Disable)
...
EHS_FB_RUN_FUNCTION(GUI_keypress_hit)
{
	...
	if(EhsGUIKbHitChar>=32&&EhsGUIKbHitChar<=126){
		...
		if(ctrlkey > -1 ){
			...
			EHS_FB_FINISH(2);
			//printf("GUI_keypress_hit:%d [%x] - out=%d\n",EhsGUIKbHitChar,EhsGUIKbHitChar,ctrlkey);
		}
	}
...
void EhsGUIKbClearCBs(void ) {
	...
	//printf("\n\n\n\n Clearing the key event handler linked list\n\n\n\n");
}
