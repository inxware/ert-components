...
EHS_FB_FUNCTION_ENTRY("start", time_clock_start)
EHS_FB_FUNCTION_ENTRY("stop", time_clock_stop)
EHS_FB_FUNCTION_ENTRY("tick", time_clock_tick)
...
EHS_FB_RUN_FUNCTION(time_clock_start)
{
	...
	if ((pTimeClock->bRetriggerable) ||(!EhsTimer_running(&(pTimeClock->xTimer)))) //we can restart a running timer to allow for new parameters to e read in
	{
		...
		/* start the timer */
		//printf("++++Calling Set time from =time_clock_start\n");
