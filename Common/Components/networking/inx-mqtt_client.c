...
// implement a null function for dynamically testing there is no Devman Support if it is not enabled
#if !defined(EHS_DEVMAN_SUPPORT)
// make sure that this function returns NULL when devman mon is not set to mqtt
//#warning building with stubbed Devman Mon
...
/* @brief Iterates through the global list pointed to by gpFirstWidget - oh really!
   todo2025: describe what this is for not how it works unless that is unclear from the code.
*/
