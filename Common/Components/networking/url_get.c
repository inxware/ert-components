...
EHS_FB_FUNCTION_ENTRY("savecookies", UrlGet_savecookies)
EHS_FB_FUNCTION_ENTRY("clearcookies", UrlGet_clearcookies)
EHS_FB_FUNCTION_ENTRY("getmoredata", UrlGet_getmore)
...
//#define EHS_DEBUG_URLGET
	...
	printf("url get init parameters\n  pUrlGet->szURL(%x)=[%s]\n "
...
EHS_FB_THREAD_FUNCTION(GetUrl_thread) {
	...
	if (!pUrlGet->bSendCookies) { // snd some specfic cookies not in our cookie files
		///\todo - does this work? _printf("Not setting cookies.. %d\n",pUrlGet->bSendCookies);
...
#ifdef EHS_URL_DEBUG
		_printf(" GetUrl_thread:No need to do URL Get - file got OK\n");
#endif
	...
	if (pUrlGet->write_data_buffer_struct->filehandle) { /* write the time stamp data */
		_printf("Closing %s[%s]\n",pUrlGet->szSavePath,szTempFilePath2);
		EhsFclose(pUrlGet->write_data_buffer_struct->filehandle);
		pUrlGet->write_data_buffer_struct->filehandle = NULL;
	...
	if (ret32 == 200) {
		...
		if (pUrlGet->bWriteToFile) {
			...
			if (request_config_error == EHS_FALSE) {
				...
				_printf("Downloaded and renaming [%s]-to-[%s]\n",szTempFilePath2,pUrlGet->szActualSavePath);
				...
				if (remotefiletimestamp == 0) { // we haven't got the time from the header test - first time file is written so stamp it with remote
					_printf("Getting a new time stamp ....\n");
					success = curl_easy_getinfo(PURLLGET_CURL, CURLINFO_FILETIME, &remotefiletimestamp); //returns seconds since 1970.
					_printf("Got a time stamp of %d (suc=%d)\n",(int)remotefiletimestamp,success);
				}
				...
				if (remotefiletimestamp > 0 ) { // time stamp the local file if we got a file or detected the file on the server
					...
					EHSH_LOG_INFO("Setting time stamp on [%s],time=%d",pUrlGet->szActualSavePath,remotefiletimestamp);
			...
			//else _printf("Don't clobber the existing file or update the time stamp");
		}
	...
	if (ret32 == 200) { // for all good events
		...
		if ( EHS_FB_OUT_CONNECTED(EHS_FB_URLGET_HEADERS)) { /* move to a function call! */
			...
			if ((success == CURLE_OK) && slist) {
				...
				while (slist && ((EhsStrlen(slist->data) + EhsStrlen(EHS_FB_OUT_S(EHS_FB_URLGET_COOKIES))) < (EHS_STRING_LENGTH_MAX-2) )) { //@todo this truncates if longer than string size...
					...
					slist=slist->next;
					//printf("Cookies are: %s\n",EHS_FB_OUT_S(EHS_FB_URLGET_COOKIES));
				}
			}
		...
		if (request_config_error == EHS_FALSE) {
			...
			_printf("Tried and failed File system with %d for %s\n ",ret32,pUrlGet->szActualSavePath);
		...
		_printf("REPORTING WHAT WENT WRONG....\n");
		EhsTPMutex_lock(EhsTPMutex_fbIO);
		if (bNeed_to_do) {
			if (bNetworkError == EHS_TRUE) { /* Used a previous version */
				_printf("Tried and failed networking with %d for %s\n ",ret32,pUrlGet->szURL);
				if (ret32 > 0 ) {
	...
	//printf("-------------------------> Exiting URL GET function <---------------------\n",PURLLGET_CURL);
	error:

	//printf("EHS_FB_THREAD_FUNCTION(GetUrl_thread) \n");
...
EHS_GLOBAL EHS_FB_RUN_FUNCTION(UrlGet_get) {
	...
	if (pUrlGet->bBusy == EHS_TRUE) {
		...
		_printf("!!!!!!****** BUSY with [%s] getting [%s]\n",pUrlGet->szURL,EHS_FB_IN_S(EHS_FB_URLGET_URL ));
		...
		if (EHS_FB_IN_CONNECTED(EHS_FB_URLGET_POST_DATA)) {
			//printf("XXXXXXXXXXXXXXXXXXXXX [%s]\n",EHS_FB_IN_S(EHS_FB_URLGET_POST_DATA));
	...
	_printf("----SENT RQUEST FOR [%s][%s]\n",pUrlGet->szURL,EHS_FB_IN_S(EHS_FB_URLGET_LOCAL_PATH));
}
...
EHS_GLOBAL EHS_FB_RUN_FUNCTION(UrlGet_abort) {
	...
	if (pUrlGet->write_data_buffer_struct) {
		...
		if (pUrlGet->bBusy) {
			...
			_printf("Setting the abort flag as busy\n");
		}
		//else printf("NOT Setting the abort flag as not busy\n");
	}
	//else printf("NOT Setting the abort flag as no data buffer\n");
...
EHS_GLOBAL EHS_FB_RUN_FUNCTION(UrlGet_savecookies){
	...
	ehs_char temp2[EHS_STRING_LENGTH_MAX];
	 if (PURLLGET_CURL && EhsStrlen(pUrlGet->szURL) ) {
		 EhsURL_EncodeURLtoCookieJarFSName(temp2,pUrlGet->szURL);
		//printf("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX   Saving Cookies to %s - from %s\n",temp2,pUrlGet->szURL);
		 if (EhsTF_tryCanonicPath(temp, EHS_RUNTIME_USERDATA_DIR, temp2,EHS_TRUE)) {//try to create the path whatever
			 ...
			 //printf("Actually Saving Cookies to %s\n",temp);
		 ...
		 //else printf("Couldn't create PATH %s\n",temp);
	 }
	 //printf("PURLLGET_CURL && EhsStrlen(pUrlGet->szURL = false\n");
