...
EHS_FB_FUNCTIONS_START(DevmanPlayer)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_START_CHECKING, DevmanPlayer_start_checking) 
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_STOP_CHECKING, DevmanPlayer_stop_checking)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_CHANGE_URL, DevmanPlayer_change_url)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_RECONFIGURE, DevmanPlayer_reconfigure)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_TRACK_CHANGED, DevmanPlayer_track_changed)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_PASS_THRU_NEXT, DevmanPlayer_pass_thru_get_next)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_PASS_THRU_SEND, DevmanPlayer_pass_thru_send_next)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_OUT, DevmanPlayer_out)
				EHS_FB_FUNCTION_ENTRY(EHS_FB_DEVMAN_PLAYER_RESET, DevmanPlayer_reset)
...
static int traceback2(lua_State *L) {
	printf("log: Error - devman-player server inx call failed\n");
	return 1;
}
#endif
...
int getAttributeValue(ehs_char* szAttribute, ehs_char* szXml, ehs_char* szTempStr2) {
	...
	if (pTempStr3 = EhsStrstr(szXml, szAttribute)) {
		...
		//printf("XXXX[%s][%s][%x]\n",szAttribute, pTempStr1,pTempStr1);
...
EHS_FB_THREAD_FUNCTION(DevmanPlayer_thread) {
	...
	//printf("EHS_FB_THREAD_FUNCTION(DevmanPlayer_thread) \n");
...
//#endif
			...
			if (http_no == 200) {
				...
				//printf("pDevmanPlayer->szXml[%x]=%s\n",pDevmanPlayer->szXml,pDevmanPlayer->szXml);
			}
			else {
...
void LoadStateFromFile(struct EhsFbDevmanPlayerStruct* pDevmanPlayer) {
	...
	if (file = Ehs_UserFopen("configs/devman-player/PlayListURL.cfg", "r")) {
		...
		EhsCopyFileNameFromURL(pDevmanPlayer->szPlaylistFile,
				pDevmanPlayer->szPlaylistURL);
		//printf("XXXXXXX=configs/devman-player/PlayListURL.cfg=URL =%s, file=%s\n",pDevmanPlayer->szPlaylistURL,pDevmanPlayer->szPlaylistFile);
		EhsFclose(file);
	}
	else {
		/* Do nothing */
...
/*
	printf("Media Player Parameters after config load:\n");
	printf("pDevmanPlayer->szUrl=%s\n",pDevmanPlayer->szUrl);
	printf("pDevmanPlayer->szLocalMediaPath=%s\n",pDevmanPlayer->szLocalMediaPath);
	printf("pDevmanPlayer->iPingPeriod=%d\n",pDevmanPlayer->iPingPeriod);
...
EHS_GLOBAL EHS_FB_RUN_FUNCTION(DevmanPlayer_out) {
	...
	if (XML_length > 5 ) {
		...
		if (zTempStr2) {
			...
			//printf("New PLAYLIST - Media Source=%s - Playlist=%s\n",pDevmanPlayer->szMediaSrcUrl,pDevmanPlayer->szPlaylistURL);
		}
	...
	if (pDevmanPlayer->bdoPlayListLoad || pDevmanPlayer->bResetingMedia || nNewPlaylistFromDevMan) { // if no new playlist and we already have one then retry playing it.
		//printf(" pDevmanPlayer->szPlaylistURL=%s\npDevmanPlayer->szPlaylistFile=%s\n",pDevmanPlayer->szPlaylistURL,pDevmanPlayer->szPlaylistFile);
