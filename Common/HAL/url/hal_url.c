...
//#define DONT_____EHS_NETWORKING_NO_CA_CHECK
...
long EhsHURLdoRequest(CURL *curl) {
	...
	if (curl) {
		...
		if (CURLE_OK == success) {
			//printf("BEFORE DoingEHSURLdoRequest returned %d",ret32);
			...
			//printf("AFTER = DoingEHSURLdoRequest returned %d",ret32); // valgrind doesn't like ret32 after the above call
