...
EhsTVSurfaceClass* doConvertRGBA8(png_structp PngPtr, png_infop InfoPtr)
{
	...
	if (pRet)
	{
		...
		EhsGraphicsColourClass* pPixels = EhsTVSurface_pixels(pRet); /* allow pixels to be accessed */
		png_bytep* row_pointers = png_get_rows(PngPtr, InfoPtr);	/* get raw image */
		//printf("Created: W=%d,H=%d\n",InfoPtr->width,InfoPtr->height);
		//printf( "pixels = %d x %d \n",(int)pRet->fmt.A1.nWidth,(int)pRet->fmt.A1.nHeight);
		rowbytes= png_get_rowbytes(PngPtr, InfoPtr);
		//printf( "row bytes = %d\nwidth = %d\nheight = %d\n",rowbytes,png_get_image_width(PngPtr, InfoPtr),png_get_image_height(PngPtr, InfoPtr));
