...
EHS_GLOBAL void* EhsHMem_Alloc(ehs_uint32 nSizeToAllocate, const char* fileName, ehs_uint32 lineNumber)
{
	...
	//printf("EhsHMem_tempAlloc for %s =%d - actual=%d\n",fileName,nSizeToAllocate,nAmount);
...
EHS_LOCAL void EhsL_checkSmall(const EhsLMemSmallItemType* pSmall) {
	...
	if (!pData) {
		...
		printf("%s:%i \n",pSmall->fileName,pSmall->lineNumber);
	}
...
EHS_LOCAL void EhsL_checkQuick(const EhsLMemQuickItemType* pQuick) {
	...
	if (!pData) {
		...
		printf("%s:%i \n",pQuick->fileName,pQuick->lineNumber);
	}
...
void EhsHMem_init(void) {
	...
	EhsLMem.nQuickOffset = (ehs_uint8)((ehs_uint8*)&(xQuick.nMem) - (ehs_uint8*)&xQuick);
	EhsLMem.nSmallOffset = (ehs_uint8)((ehs_uint8*)&(xSmall.nMem) - (ehs_uint8*)&xSmall);
#ifdef EHS_CONFIG_MEM_CHECK
	EhsLMem.nUsed = 0u;
#endif
}
