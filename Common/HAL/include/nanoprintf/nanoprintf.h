...
// Pick reasonable defaults if nothing's been configured.
  ...
  #define NANOPRINTF_USE_LARGE_FORMAT_SPECIFIERS 1 // Only needed for the 64-bit hash integer
...
static EHS_MEMORY_ATTRIB int npf_parse_format_spec(char const *format, npf_format_spec_t *out_spec) {
...
static NPF_NOINLINE EHS_MEMORY_ATTRIB int npf_utoa_rev(
...
static NPF_FORCE_INLINE EHS_MEMORY_ATTRIB npf_double_bin_t npf_double_to_int_rep(double f) {
...
static EHS_MEMORY_ATTRIB int npf_ftoa_rev(char *buf, npf_format_spec_t const *spec, double f) {
  char const *ret = NULL;
...
static EHS_MEMORY_ATTRIB void npf_bufputc(int c, void *ctx) {
  npf_bufputc_ctx_t *bpc = (npf_bufputc_ctx_t *)ctx;
  if (bpc->cur < bpc->len) { bpc->dst[bpc->cur++] = (char)c; }
}

static EHS_MEMORY_ATTRIB void npf_bufputc_nop(int c, void *ctx) { (void)c; (void)ctx; }
...
static EHS_MEMORY_ATTRIB void npf_putc_cnt(int c, void *ctx) {
...
EHS_MEMORY_ATTRIB int npf_vpprintf(npf_putc pc, void *pc_ctx, char const *format, va_list args) {
...
EHS_MEMORY_ATTRIB int npf_vsnprintf(char * NPF_RESTRICT buffer,
                  size_t bufsz,
                  char const * NPF_RESTRICT format,
                  va_list vlist) {
