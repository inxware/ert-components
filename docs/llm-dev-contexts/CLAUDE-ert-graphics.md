...
#### Imperative writes destroy QML bindings — use interchange properties

**Every eRT write into QML is a `QObject::setProperty()` call** (`ertqt.cpp`, `set_property()`), reached via `EhsUI_set_property_*` from the `ui_data_*` blocks or via `ertqt_set_property_*` from the Mode B widget path. In QML an imperative write **permanently destroys any declarative binding on the property it lands on**. A property declared `value: someExpression` loses that expression on the first write and never tracks its source again.

This is standard documented QML behaviour, not a defect in the HAL — but it dictates how an app must be wired.

**Signals are not affected.** `connect_signal_by_name()` only ever adds a receiver (a `QSignalMapper` parented to the sender, then a lambda). It never disconnects anything, so a `ui_event` subscription cannot displace QML-side `on<Signal>` handlers or `Connections` blocks. Conflicts are a property-level phenomenon only.

**The rule: eRT writes inert interchange properties; visuals bind from them.**

```qml
Item {
    id: appRootId

    // eRT interchange surface — ui_data_* blocks with Object="_qml_root_object_"
    // write these. Plain properties, constant initialisers, single writer.
    property int  speed: 0
    property bool warning: false

    // Visuals bind FROM the interchange properties. Nothing writes SpeedGauge's
    // own `value` imperatively, so this binding survives.
    SpeedGauge { value: appRootId.speed }
}
```

Writing `SpeedGauge.value` directly from eRT would work exactly once, then the gauge would freeze.

Rules for anything in the interchange block:

- Keep it a plain property with a constant initialiser (`: 0`), never a binding expression.
- No `Behavior on`, no animation, no `Timeline` driving it.
- Never target it from a State's `PropertyChanges` — states re-apply on entry and revert on exit, overwriting whatever eRT wrote.
- **Single writer.** If QML also assigns it, the two writers fight and the last one wins.

**The same trap inside pure QML.** A control that both binds to a model and writes back to it destroys its own binding the moment the user touches it — the control writes its own property while being dragged or clicked:

```qml
Slider {
    value: UIState.forkLift              // BROKEN: destroyed on first drag
    onValueChanged: UIState.forkLift = value
}
```

Use a `Binding` element for the model → control direction (it re-applies on every change and so survives the control's own writes), and an interaction-only signal for control → model (`onMoved` on Slider, `onToggled` on CheckBox — not `onValueChanged` / `onCheckedChanged`, which also fire for the Binding's own write and echo):

```qml
Slider {
    id: forkHeightSlider
    onMoved: UIState.forkLift = value
}
Binding { target: forkHeightSlider; property: "value"; value: UIState.forkLift }
```

A worked example of both halves lives in the Forklift3D demo app — `app.qml` documents its interchange block, and `SettingsPage.qml` carries the corrected Slider/CheckBox pattern.

**Symptom to recognise:** two things appear to drive one value and only the first to fire keeps working, the other silently stops. That is binding destruction, and it is often mistaken for an event-dispatch bug on the eRT side.

**Also worth knowing:** `.ui.qml` files (Qt Design Studio) may not contain imperative JavaScript. A signal handler with a body placed in one will be rejected or stripped by QDS, which produces a "handler never fires" symptom with no C++ involvement at all. Handlers with logic belong in plain `.qml` files.
