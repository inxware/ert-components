...
deployable binary in one step (usually named `ehs.exe`). 
`make targetenv` stages files and other assets 
`make targetenv_pack` selects a packaging tool (basedon config), but no additional binary linking occurs.

**Platforms Examples:** Linux, Windows

---

### Pattern B — Library: `make all` then link and packag with an SDK.

This is more typical of MCU/RTOS builds, but is also used in "agent" mode for large platforms like Unity3D or application SDKs like Android.

`make all` compiles eRT source to an intermediate artifact (`.a`, `.lib`, `.elf`,
or `.so` ELF).  A separate `make targetenv_pack_*<_docker>` step uses a
target-specific toolchain or SDK to perform the final linking and/or image
generation.  

This is necessary when:

- The target SDK owns the final link step (e.g. XMOS, Arduino CLI library)
- Multiple binary components must be merged into a flashable image (e.g. ESP32, Zephyr)
