...
model_sample_rate, model_samples = wavfile.read('C:\\Users\\Rory\\Music\\Test playlists\\Model.wav')
sample_rate, samples = wavfile.read('C:\\Users\\Rory\\Music\\Test playlists\\Test.wav')
fftsize = 4096
...
clip_spectrogram = test_sum > 50000
...
model_clip_spectrogram = model_sum > 50000
...
#    times.resize(model_times.shape)
#
#else:
#    model_times.resize(times.shape)
#view_length = model_times
#print(times.shape)
...
#Needs to be length of the *longer* of the two without breaking by having them be the same length.
#Possible to append the shorter one to make it as long as the longer?
...
if ( 1 ) :
   ...
   #ax[0].pcolormesh( spectrogram[1:frequencyrange,test_start_index:test_start_index+10000],)
    #ax[1].pcolormesh( model_spectrogram[1:frequencyrange,model_start_index:model_start_index+10000])
    # ax[0].yscale('log')
...
index = 0  
for tone in TestLUT:
    # note the next line needs reverse look up if closest index to times
    start_index = numpy.min(numpy.where(times >= tone[0] ))
...
x = npTestLUT[:,0]
y = npTestLUT[:,4]
plt.plot(x.astype(numpy.float),y.astype(numpy.float),'.')
a = npTestLUT[:,1]
b = npTestLUT[:,4]
plt.plot(a.astype(numpy.float),b.astype(numpy.float),'.')
#plt.show()