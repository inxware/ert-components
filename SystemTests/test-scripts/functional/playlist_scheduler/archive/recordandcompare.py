...
#requests.post("http://192.168.2.202:8000/index.php")

testrecording = sd.rec(int(duration * fs), samplerate = 8000, channels=1, blocking=True)
wavfile.write('C:\\Users\\inx limited\\Music\\Test playlists\\Test.wav', fs, testrecording)

sample_rate, samples = wavfile.read('C:\\Users\\inx limited\\Music\\Test playlists\\Test.wav')
...
if ( len(sys.argv) > 1 and sys.argv[1] == 'create_template') :
    ...
    model_sample_rate, model_samples = wavfile.read('C:\\Users\\inx limited\\Music\\Test playlists\\newmodel8K.wav') 
...
clip_spectrogram = test_sum > 0.8
model_clip_spectrogram = model_sum > 0.8
print('model clip spec1 =', numpy.shape(model_clip_spectrogram))
print('model clip spec1 where =', numpy.shape(numpy.where(model_clip_spectrogram > 0.5)))

model_clip_indexes_greaterthan = numpy.where(model_clip_spectrogram > 0.5)
print(model_clip_indexes_greaterthan)
print(numpy.shape(model_clip_indexes_greaterthan))
print(numpy.shape(model_clip_indexes_greaterthan[0]))
print(model_clip_indexes_greaterthan[0])

if len(model_clip_indexes_greaterthan[0]) > 0:
    model_start_index = numpy.min(numpy.where(model_clip_spectrogram > 0.5))
else: sys.exit("Could not find model white noise")
print(clip_spectrogram)
print('clip spec1 =', numpy.shape(clip_spectrogram))
print('clip spec1, where =', numpy.shape(numpy.where(clip_spectrogram > 0.5)))
test_clip_indexes_greaterthan = numpy.where(clip_spectrogram > 0.5)
if len(test_clip_indexes_greaterthan[0]) > 0 :
    test_start_index = numpy.min(numpy.where(clip_spectrogram > 0.5))    
...
spectrogram_diff = spectrogram[:,test_start_index:test_start_index+view_length]-model_spectrogram[:,model_start_index:model_start_index+view_length]
if ( 1 ) :
...
for tone in TestLUT:
 ...
 #   ts.test_cases.append(test_cases)
