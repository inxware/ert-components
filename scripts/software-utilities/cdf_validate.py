    ...
    try:
        ...
        for i, line in enumerate(content.split('\n'), start=1):
            ...
            if 'EHS_FB_FUNCTIONS_START' in line:
                ...
                if match:
                    function_block_name = match[0]
                    function_blocks[function_block_name] = dict()
                else:
                    print(f"Failed to parse function block name in line: {line}")
            elif 'EHS_FB_FUNCTIONS_END' in line:
                function_block_name=None
            
            if function_block_name is not None:
                if 'EHS_FB_FUNCTION_ENTRY' in line: 
                    function=re.search(r'EHS_FB_FUNCTION_ENTRY\("([^"]+)",\s*([^,]+),\s*([^)]+)\)', line)
                    if function:
                        function_blocks[function_block_name][function.group(1)]=str(int(function.group(2),16)) # read hex as int
                        #print(f"  Name: {function.group(1)}, Value: {function.group(2)}, Function: {function.group(3)}")
    except:
