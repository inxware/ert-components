...
#   7  COMMIT/TAG/PUSH   per repo, with confirmation and an optional GUI review.
#                        Tags both sides only after the push succeeds.
#
...
if [ "$DRY" -eq 1 ]; then
    ...
    for _pair in "ert-components|${GH}/ert-components|${ROOT}" \
                 "ert-contrib-middleware|${GH}/ert-contrib-middleware|${ROOT}/../ert-contrib-middleware" \
                 "ert-build-support|${GH}/ert-build-support|" \
                 "ert-kernels|${GH}/ert-kernels|${ROOT}/../ert-kernels" \
                 "apps|${GH}/apps|${ROOT}/../apps"; do
        ...
        if [ "$_n" -eq 0 ]; then
            good "${_name}: nothing changed, nothing to commit"
            continue
        fi

        echo
        warnl "${_name}: ${_n} path(s) changed in the mirror"
        git -C "$_mir" status --short | head -12 | sed 's/^/      /'
        [ "$_n" -gt 12 ] && note "      ... $((_n - 12)) more"

        if command -v gitk >/dev/null 2>&1 || command -v git-gui >/dev/null 2>&1; then
            printf '   Open a GUI to review %s? [y/N] ' "$_name"
            read -r _ans </dev/tty || _ans=n
            case "$_ans" in
                [Yy]*)
                    # git gui shows the working tree; gitk shows history. The
                    # working tree is what matters here, so prefer git gui.
                    if command -v git-gui >/dev/null 2>&1; then
                        git -C "$_mir" gui >/dev/null 2>&1
                    else
                        gitk --all >/dev/null 2>&1 &
                        note "      gitk opened in the background"
                    fi
                    ;;
            esac
        fi
