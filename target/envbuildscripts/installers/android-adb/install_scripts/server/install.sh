...
if [ "$NETWORK_HARDWIRED_HOSTS" != "" ];then 
	echo "Setting NETWORK_HARDWIRED_HOSTS = $NETWORK_HARDWIRED_HOSTS"
	shell_exec "echo -e  ${NETWORK_HARDWIRED_HOSTS} >> /system/etc/hosts" || exit 1
	echo "Done!"
fi


if [ "$NETWORK_NTP_SERVER" != "" ];then
	echo "Setting NETWORK_NTP_SERVER = $NETWORK_NTP_SERVER"
	shell_exec "settings put global ntp_server ${NETWORK_NTP_SERVER}" || exit 1
	echo "Done!"
fi
