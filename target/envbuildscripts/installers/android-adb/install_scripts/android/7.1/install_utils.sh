...
rmtemp(){
	$ADB shell 'test -d '${TEMP_DIR}' && rm -r '${TEMP_DIR} || :
	$ADB shell 'test -f '${TEMP_DIR}' && rm  '${TEMP_DIR} || :
}
...
starthomeapp(){
	PACKAGE=$1
	INTENT=$2
	$ADB shell 'cmd package set-home-activity '${PACKAGE}'/'${INTENT} || exit 1
	sleep 1
	$ADB shell 'am start -a android.intent.action.MAIN -c android.intent.category.HOME' || exit 1
}

startdownloader(){
	$ADB shell 'am startservice com.utils.downloader/.DownloadService' || exit 1
}
...
check_android_version(){
	...
	if [ "$VERSION" == "7.1.2" ]; then
		...
		echo "Target device android version ($VERSION) doesn't match configuration (7.1.2)."
		exit 1
	fi
}
