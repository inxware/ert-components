...
#	export ANDROID_INSTALLER_DIR=$( pwd )/../../envbuildscripts/installers/android-adb
#	export INSTALL_DIR=$( pwd )
fi
echo "INSTALLER DIR ==> $ANDROID_INSTALLER_DIR"
echo "ANDROID OS PATCHES & SUERVISOR ==> $INSTALL_DIR"
...
function avl() {
	ls ${ANDROID_INSTALLER_DIR}/install_scripts/android
}

function prl() {
	ls ${ANDROID_INSTALLER_DIR}/install_scripts/product
}

function pll() {
	ls ${ANDROID_INSTALLER_DIR}/install_scripts/platform
}

function snl() {
	echo "Ignore this it's gone at last!!!"
	ls ${ANDROID_INSTALLER_DIR}/install_scripts/server
}
...
export INSTALL_PRODUCT_DIR=${ANDROID_INSTALLER_DIR}/install_scripts/product/${PRODUCT}
export SET_UP_PLATFORM_DIR=${ANDROID_INSTALLER_DIR}/install_scripts/platform/${PLATFORM}
export ANDROID_VERSION_DIR=${ANDROID_INSTALLER_DIR}/install_scripts/android/${ANDROID_VERSION}

echo "-------------------------------------"
echo "Installing eRT and the supervisor as " 
...
#	INSTALL_SERVER_DIR=${ANDROID_INSTALLER_DIR}/install_scripts/server/${SERVER_NAME}
#	if ! [ -d "$INSTALL_SERVER_DIR" ]; then
#		echo "Unknown target server $SERVER_NAME at $INSTALL_SERVER_DIR"
#		exit 1
#	fi
#else
#	# set server based on product name
#	INSTALL_SERVER_DIR=${ANDROID_INSTALLER_DIR}/install_scripts/server/${PRODUCT}
#	if ! [ -d "$INSTALL_SERVER_DIR" ]; then
#		INSTALL_SERVER_DIR=""
#	fi
#fi
...
#source $ANDROID_VERSION_DIR/install_utils.sh || exit 1
...
#we can only connect to one device as root so disconnect to any others
adb disconnect ||: 
sleep 10
...
# install downloader
#pushd $ANDROID_INSTALLER_DIR || exit 1
source ${EHS_ROOT}/target/envbuildscripts/installers/android-adb/install_scripts/install_utils.sh || exit 1
...
if [ -d "$INSTALL_APK_DIR" ]; then
	...
	echo "OK - Added APKs to the install directories"
fi
