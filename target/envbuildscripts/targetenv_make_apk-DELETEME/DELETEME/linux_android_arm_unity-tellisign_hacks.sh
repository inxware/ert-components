...
# todo2023 -this should be made generic for building any unity app of the same type. 
# It should probably include the linux_android_arm_build.sh unless that is already called in the framework.
# Or should theese utilities just be added to the linux_android_arm_build.sh?

export SSHPORT=8822

if [ "${DEVMAN_SERVER_NAME}" = "" ]; then
    echo "NO DEVMAN_SERVER_NAME is set"
    exit 1
else 
...
if [ "${DEVMAN_SERVER_DOMAIN}" = "" ]; then
    echo "NO DEVMAN_SERVER_DOMAIN is set"
    exit 1
else
...
if [ "${DEVMAN_SERVER_PROTOCOL}" = "" ]; then
    echo "NO DEVMAN_SERVER_PROTOCOL is set"
    exit 1
else
...
#todo2023 -the following can be set in the devman/config.mk but shouldn't be mandatory as we don't awlways post stuff 
...
source ${EHS_ROOT}"/target/envbuildscripts/targetenv_make_apk/targetenv_make_apk_utils.sh"
source ${EHS_ROOT}"/target/envbuildscripts/targetenv_make_apk/targetenv_make_apk_setup.sh"


# This checks the 63 bit lib is available and copies into 
# Downloads Android Studio
# Downloads Unity if it is not installed (it is zipped - as a raw file on our Repo server via ssh )
#
...
#todo 2023 This probably needs chucking away we need to getthe app from the staging directiory as we have already done this.
SetupTargetEnv_CopyEHSTools(){
    ...
    if [ -d "$TOOLS_DIR" ] && [ -d "$ANDROID_STUDIO_TOOLS_PATH" ]; then
        echo "Copying the Unity signage tools app to the project."
...
# The generic linux_android_arm_build.sh calls ./target/envbuildscripts/targetenv_make_apk/targetenv_make_apk_gradlew_build.sh to do the gradelbuild.
# Is below just duplicating that?
TargetEnvMakeApk_Build(){
    ...
    if grep -q "RELEASE_STORE_FILE" "$GRADLE_PROPS_FILE"; then
        ...
        echo "" >> $GRADLE_PROPS_FILE # make sure we have a new line before appending to file
    ...
    #todo2023 can't find any trace of launcher-release.apk being generated anywhere...
