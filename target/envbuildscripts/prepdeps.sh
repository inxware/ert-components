...
if  ! command -v docker &> /dev/null ; then
    ...
    read -n 1 -p "Docker has been installed, which requires you to reboot your machine or re-login so that new user permissions can take affect for ${USER}"
else
...
if [ ! -n "${SKIP_REPOS}" ];then
    ...
    if [ -e $EHS_COMPONENT_SUPPORT_LOCAL_PATH ]; then
      ...
      if [ -e $EHS_KERNEL_LOCAL_PATH ]; then
        ...
        echo "No local repo found. Cloning the ert-build-support repository $EHS_KERNEL_SERVER_PATH to $EHS_KERNEL_LOCAL_PATH"
