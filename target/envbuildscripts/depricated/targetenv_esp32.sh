...
if [ -f /.dockerenv ]; then
    ...
    export IDF_PYTHON_ENV_BASE="/opt/pyhthon_env/"
...
if test -f "$PWD/../ert-contrib-middleware/target_libs/xtensa-esp32_freertos-xtensa-esp32-elf-4.4.1/build/lib/partitions_singleapp.bin" #"$PWD/../ert-contrib-middleware/contrib/esp-idf/esp-idf-4.4.1/build/partitions.bin"
then
    cp $PWD/../ert-contrib-middleware/target_libs/${COMPONENT_BASE_TECHNOLOGIES}/build/lib/partitions_singleapp.bin $PWD/../TARGET_TREES/ehs_env-$SPECIFIC_TARGET/bin/ || echo "ERROR: Could not copy the partition table"
