...
/* Config that was in the hearod_config.h file */
...
static uint32_t gMqttConnections;
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    {
/* I expect this is not actually used in eRT, but leaving it here as areference for aall back function.

Preumably for eRT there is another callback function that handles reciving pubsub messages for each function block
...
#endif // end of disabled heatrod application code.
    }
    else
    {
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
    ...
    /* Thi is the other part of the weird way TOPICS are handled for apps */
#if 0
...
void mqttMainLoop()
{
    ...
    case MQTT_STATE_DO_CONNECT:
        if (connect)
        {
            ...
            if (gUseTLS)
            {
                ...
                if (TLS_Init(
                        ...
                        inxEHSFilesGetData(inxEHSFilesFind(rootCAFileName))) != 0)
                {
                    /* failed? */
        ...
        {
            /* todo slang2022 - we need to bypass this state if TLS is not enabled. or fake it with a stub function that saysit's done?*/
    ...
    case MQTT_STATE_DO_DISCONNECT:
        ...
        if (gMqttConnectionAttempts >= CONFIG_MQTT_MAX_CONNECTION_ATTEMPTS)
        {
            ...
            /* PMLD Sorry I clobbered this - will need to be put back I expect ! */
 ...
 */
u8_t mqtt_client_is_handshaking(mqtt_client_t *client)
{
    if (client == NULL)
    {
