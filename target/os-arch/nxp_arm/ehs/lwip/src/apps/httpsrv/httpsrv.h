...
/* Use this functions for redirecting the path which was the original request,
 * but needs to be redirected due to the current state of the device.
 * The original path is passed as an argument. The function should returns a
 * pointer to the new path.
 * Do not set this function if redirection is not required. */
