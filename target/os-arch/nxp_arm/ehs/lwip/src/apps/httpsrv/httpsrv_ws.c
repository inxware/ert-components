...
static uint32_t ws_process_api_calls(WS_CONTEXT_STRUCT *context)
{
    ...
    while (active == 0)
    {
        ...
        //PBB kill off web socket if rest of server is being closed
        if (active == -1 || context->session->valid!=HTTPSRV_VALID)
        {
            retval = WS_ERR_FAIL;
            break;
        }
...
static int ws_send_frame(WS_CONTEXT_STRUCT *context, WS_FRAME_STRUCT *frame)
{
    ...
    do
    {
        ...
        {
            frame->fin = false;
            frame->length = buffer->size - ws_get_header_size(frame);
        }
        /*
        ...
        {
            frame->fin = false;
            frame->length = data_length - ws_get_header_size(frame);
        }
        if (frame->fin == true)
        {
