...
int httpsrv_wait_for_conn(HTTPSRV_STRUCT *server)
{
    ...
    if (lwip_select(server->sock+1, (fd_set *)&readset, 0, 0, &timeout) == 1)
    {
...
void httpsrv_destroy_server(HTTPSRV_STRUCT *server)
{
    ...
    if (server)
    {
        if (server->valid)
        {
            ...
            if (server->sock != -1)
            {
                while(lwip_close(server->sock)!=0){
                	printf("285 failed closing http socket, delaying\n");
                	vTaskDelay(1000);
                }
                server->sock = -1;
            }
...
void httpsrv_abort(int sock)
{
	...
	while(lwip_close(sock)!=0){
		printf("575 failed closing http socket, delaying\n");
		vTaskDelay(1000);
	}
}
...
{
	LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_sendfile: start"));
    char *ext;
    int length;
    char *buffer;
    ...
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_sendfile: end"));
    return (retval);
}
...
int32_t httpsrv_read(HTTPSRV_SESSION_STRUCT *session, char *dst, int32_t len)
{
    ...
    {
        int received;
        //PBB thread can lock up here if we haven't already sent something back
