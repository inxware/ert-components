    ...
    if (server)
    {
        ...
        if (server->sock != -1)
        {
            ...
            while(lwip_close(server->sock)!=0){
            	//SW_DIAG_printf("79 failed closing http socket, delaying\n");
				vTaskDelay(1000);
			}
            //SW_DIAG_printf("delaying so network stuff can happen\n");
            sys_sem_signal(&server->ses_cnt);
            vTaskDelay(2000);
            //SW_DIAG_printf("finished delaying so network stuff can happen\n");
        }

        while (server->server_tid) /* Wait for task completition.*/
        {
        	//SW_DIAG_printf("waiting for http server to close\n");
            //taskYIELD();
            vTaskDelay(2000);
        }
        httpsrv_mem_free(server);
        LWIP_PLATFORM_DIAG(("http server released"));
    }
}
