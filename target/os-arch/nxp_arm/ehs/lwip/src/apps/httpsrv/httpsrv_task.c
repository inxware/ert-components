...
char* (*httpsrv_redirect_function)(char*) = NULL;
void httpsrv_set_redirect_function(char* (*function)(char*)){
	httpsrv_redirect_function = function;
}
...
void httpsrv_server_task(void *arg)
{
    ...
    while (1)
    {
        ...
        LWIP_DEBUGF(DEBUG_HTTP_SRV,("52 sys_arch_sem_wait"));
        sys_arch_sem_wait(&server->ses_cnt, 0);

        /* Get socket with incoming connection (IPv4 or IPv6) */
        LWIP_DEBUGF(DEBUG_HTTP_SRV,("55 httpsrv_wait_for_con"));
        int connsock = httpsrv_wait_for_conn(server);
        LWIP_DEBUGF(DEBUG_HTTP_SRV,("57 connsock=%d",connsock));
        if (server->valid == HTTPSRV_INVALID)
        {
        	LWIP_DEBUGF(DEBUG_HTTP_SRV,("60 httpsrv_invalid so breaking"));
            break;
        }
        if (connsock == -1)
        {
        	LWIP_DEBUGF(DEBUG_HTTP_SRV,("65 vTaskDelay"));
        	sys_sem_signal(&server->ses_cnt); //so we don't block forever when we come back round
        	sys_msleep(1000);//vTaskDelay(1000); //taskYIELD();
        }
        else
        {
        	//LWIP_DEBUGF(DEBUG_HTTP_SRV,("70 httpsrv_accept"));
        	//UBaseType_t uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL );
        	//LWIP_DEBUGF(DEBUG_HTTP_SRV,("http_task stack bytes free=%d",uxHighWaterMark*4));
            ...
            else
            {
                ...
                if (i != server->params.max_ses)
                {
                    ...
                    if (session)
                    {
                        ...
                        if (ses_param != NULL)
                        {
                            if (ERR_OK == httpsrv_ses_init(server, session, new_sock))
                            {
                                ...
                                {
                                	LWIP_DEBUGF(DEBUG_HTTP_SRV,("131 failed to create http session task"));
                                	...
                                	LWIP_DEBUGF(DEBUG_HTTP_SRV,("142 created task i=%d",i));
                                }
                            }
                            else
                            {
                            	LWIP_DEBUGF(DEBUG_HTTP_SRV,("142 failed to init http session"));
                                httpsrv_ses_close(session);
                                httpsrv_ses_free(session);
                                sys_sem_signal(&server->ses_cnt);
                                httpsrv_mem_free(ses_param);
                        ...
                        {
                        	LWIP_DEBUGF(DEBUG_HTTP_SRV,("152 failed to alloc http session param"));
                            httpsrv_ses_close(session);
                            httpsrv_ses_free(session);
                            sys_sem_signal(&server->ses_cnt);
                        }
                    ...
                    {
                    	LWIP_DEBUGF(DEBUG_HTTP_SRV,("161 failed to alloc http session"));
                ...
                {
                	LWIP_DEBUGF(DEBUG_HTTP_SRV,("169 hit max session"));
                    httpsrv_abort(new_sock);
                    sys_sem_signal(&server->ses_cnt);
                }
            }
    ...
    /* Server release.*/
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("171 destroying server"));
    httpsrv_destroy_server(server);
    server->server_tid = 0;
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("174 http_task done"));
    vTaskDelete(NULL);
}
...
static void httpsrv_session_task(void *arg)
{
	LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_session_task"));
    ...
    while (session->valid)
    {
    	...
    	//LWIP_DEBUGF(DEBUG_HTTP_SRV,("201 httpsrv_session_task"));
        session->process_func(server, session);
        //UBaseType_t uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL );
        //LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_session_task stack bytes free=%d",uxHighWaterMark*4));
        taskYIELD();
    }
    httpsrv_ses_close(session);
    httpsrv_ses_free(session);
    *ses_param->session_p = NULL;
    ...
    httpsrv_mem_free(ses_param);
    sys_sem_signal(&server->ses_cnt);
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_session_task delete"));
    vTaskDelete(NULL);
}
...
static void httpsrv_ses_close(HTTPSRV_SESSION_STRUCT *session)
{
    if (session != NULL)
    {
        ...
        if (session->sock != -1)
        {
            ...
            while(lwip_close(session->sock)!=0){
				LWIP_DEBUGF(DEBUG_HTTP_SRV,("378 failed closing session socket, delaying"));
				vTaskDelay(1000);
			}
            session->sock = -1;
        }
...
void httpsrv_http_process(void *server_ptr, void *session_ptr)
{
    ...
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_http_process: session->state=%d",session->state));
...
static HTTPSRV_SES_STATE httpsrv_req_do(HTTPSRV_STRUCT *server, HTTPSRV_SESSION_STRUCT *session)
{
    ...
    {
    	//PBB we only want to allow one web socket session at a time
		//so iterate through sessions and see if any already have have httpsrv_plugin_run as their process_func
		for (int i = 0; i < server->params.max_ses; i++)
		{
			if (server->session[i] != NULL)
			{
				if(server->session[i]->process_func==httpsrv_plugin_run){
					session->response.status_code = HTTPSRV_CODE_METHOD_NOT_ALLOWED;
					goto EXIT;
				}
			}
    ...
    // Check if the original request needs to be redirected.
    char* httpsrv_path = full_path;
    if(httpsrv_redirect_function != NULL){
    	httpsrv_path = httpsrv_redirect_function(full_path);
    }
    session->response.file = HTTPSRV_FS_open(httpsrv_path);
...
static HTTPSRV_SES_STATE httpsrv_response(HTTPSRV_STRUCT *server, HTTPSRV_SESSION_STRUCT *session)
{
    ...
    LWIP_DEBUGF(DEBUG_HTTP_SRV,("httpsrv_response: status_code=%d method=%d",session->response.status_code,session->request.method));
