...
uint8_t ustrneq(const char* string1, const char* string2, uint8_t n) {
  ...
  //only return true if we are at the end of both strings
  if(string1[utz_i]=='\0' && string2[utz_i]=='\0'){
    return UTRUE;
  }
  return UFALSE;
}
...
void unpack_rule(const urule_packed_t* rule_in, uint8_t cur_year, urule_t* rule_out) {
  ...
  //printf("PBB 171 %d-%d-%dT%d:%d\n",rule_out->date.year,rule_out->date.month,rule_out->date.dayofmonth,rule_out->time.hour,rule_out->time.minute);
}
...
char get_current_offset(uzone_t* zone, udatetime_t* datetime, uoffset_t* offset) {
  ...
  if (last_zone != zone->src || last_year != datetime->date.year) {
    //remove any previously cached rules
  ...
  //printf("PBB 235 %d-%d-%dT%d:%d\n",rule->date.year,rule->date.month,rule->date.dayofmonth,rule->time.hour,rule->time.minute);
