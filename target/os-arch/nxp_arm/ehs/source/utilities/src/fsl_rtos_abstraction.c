...
//these definitions are here to prevent compiler warnings
uint64_t microseconds_convert_to_ticks(const uint32_t ticks);
uint64_t microseconds_get_ticks();
void microseconds_delay(const uint32_t delay);
