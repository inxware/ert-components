...
//                                                                     &propertyStore->flashSectorSize[kFalshIndex_DFlash]);                 
...
#else
    flashStart =0x1fff0000;//TODO PBB just hardcoding these for now //__ROM_START;
    flashEnd = 0x20000000;//TODO PBB just hardcoding these for now__ROM_END;
    assert(flashEnd);
...
#endif
#endif // #if !BL_FEATURE_HAS_NO_INTERNAL_FLASH
    //TODO PBB ignore these
    ramStart = 0x1fff0000; //TODO PBB just hard coding these for now//__RAM_START;
    ramEnd = 0x20000000; //TODO PBB just hard coding these for now//__RAM_END;
    assert(ramEnd);
    ...
    //TODO I'm guessing we don't need
...
status_t bootloader_property_get(uint8_t tag, uint32_t id, const void **value, uint32_t *valueSize)
{
    ...
    switch (tag)
    {
        ...
        case kPropertyTag_MaxPacketSize:
        	...
        	//TODO PBB I'm sure we don't need this
