                              ...
                              ftfx_swap_state_config_t *returnInfo)
{
    ...
    {
    	HW_DIAG_printf("kStatus_FTFx_InvalidArgument\n");
    	return kStatus_FTFx_InvalidArgument;
    }

    if (address & (FSL_FEATURE_FLASH_PFLASH_SWAP_CONTROL_CMD_ADDRESS_ALIGMENT - 1))
    {
    	HW_DIAG_printf("kStatus_FTFx_AlignmentError\n");
    ...
    {
    	HW_DIAG_printf("Not in Lower half\n");
    	return kStatus_FTFx_SwapIndicatorAddressError;
    }

    /* Check the option. */
    //HW_DIAG_printf("CCCCCCCCCCCC\n");
    returnCode = ftfx_check_swap_control_option(option);
    if (returnCode)
    {
    	HW_DIAG_printf("bad ftfx_check_swap_control_option\n");
    	return returnCode;
    }
/*
    The flash configuration field is a 16-byte section of flash memory located at 0x400-0x40F. Values from the flash
    configuration field are copied to flash registers at reset, so they set the default values for several flash settings, most
    importantly the flash protection and system security settings.
    To prevent accidental securing of devices, the flash configuration field requires special treatment. Freescale recommends that
    by default flash programmers write the following value to the flash configuration field:
    0xFFFF_FFFF
    0xFFFF_FFFF
    0xFFFF_FFFF
    Other programming considerations
    Production Flash Programming Best Practices for Kinetis K- and L-series MCUs, Rev 1, 05/2014
    Freescale Semiconductor, Inc. 7
    0xFFFF_FFFE
    This value places the device in an unsecured state with no flash regions protected.
    Some users will want to program the flash configuration field, so an override option should be included to enable users to
    program other values into the flash configuration field. Note that changes to the flash configuration field will take effect at
    the next reset. To prevent an updated flash configuration field value that secures the processor and prevents further access to
    the processor, ensure the programmer completes all programming steps and any verification of values written before resetting
    the processor.
*/

    /* See figure 29-10 in K64P144M120F5RM manual */
    /* Some of this prep "might" might be done in ftfx_command_sequence but not as defined in the manual */
    /* Wait for previous command to complete */
    //HW_DIAG_printf("CCCCCCCCCCCC 33\n");
#if 0 // tried by PMLD - possibly advisable but needs review and testing
    while (!(FTFx->FSTAT & FTFx_FSTAT_CCIF_MASK))
            {
            }
#endif
    //HW_DIAG_printf("CCCCCCCCCCCC 44\n");
    /* Check the ACCEL/FPIOL flag and attempt to clear with 0x30*/
    if (FTFx->FSTAT & (FTFx_FSTAT_ACCERR_MASK | FTFx_FSTAT_FPVIOL_MASK)) FTFx->FSTAT = 0x30;
   // HW_DIAG_printf("CCCCCCCCCCCC 45\n");

    kFCCOBx[0] = BYTE2WORD_1_3(FTFx_SWAP_CONTROL, address);
    //HW_DIAG_printf("CCCCCCCCCCCC 46\n");
    kFCCOBx[1] = BYTE2WORD_1_3(option, 0xFFFFFFU);
    //HW_DIAG_printf("CCCCCCCCCCCC 47\n");
    //FTFx->FSTAT = 0x80; //pmld this is in the manual spec. but seems to cause an instant reset when run, It should execute the flash command but perhaps the RAM Function code called previously has already done this?
   // HW_DIAG_printf("CCCCCCCCCCCC 55\n");
    returnCode = ftfx_command_sequence(config);
    // HW_DIAG_printf("CCCCCCCCCCCC 60\n");
    //while (!(FTFx->FSTAT & FTFx_FSTAT_CCIF_MASK)) /* added as per spec */
    //        {
    //        }
    //  HW_DIAG_printf("CCCCCCCCCCCC 66\n");

    //HW_DIAG_printf("ftfx_command_sequence Returned %d (good is %d)\n",returnCode,kStatus_FTFx_Success);
...
static status_t ftfx_command_sequence(ftfx_config_t *config)
{
    ...
    /* Since the value of ARM function pointer is always odd, but the real start address
     * of function memory should be even, that's why +1 operation exist. */
volatile callFtfxRunCommand_t callFtfxRunCommand = (callFtfxRunCommand_t)((uint32_t)config->runCmdFuncAddr + 1);
    ...
    //HW_DIAG_printf("waiting for CCIF\n");
       while (!(FTFx->FSTAT & FTFx_FSTAT_CCIF_MASK)) /* pmld moved this to pply in all cases as in the ref manual spec */
           {
      // 	HW_DIAG_printf(".");
           }
      // HW_DIAG_printf("\nDone Waiting\n");
    ...
    //HW_DIAG_printf("FSTAT (2) = %x\n",registerValue);
    //HW_DIAG_printf("kStatus_FTFx_AccessError\n");
