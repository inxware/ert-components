...
EHS_GLOBAL ehs_bool EhsHThread_execute(EhsGeneralThreadFuncType pfRun, void* context,ehs_sint16 priority, ehs_sint32 stacksize)
{
    ...
    EhsTPThread thread;
    if (stacksize == EHS_THREAD_USE_DEFAULT_STACK_SIZE) stacksize = 2000; // todo use a macro for the default like for nxp
    thread = sys_thread_new("ehs", (lwip_thread_fn)pfRun, context, stacksize, CONFIG_MAIN_THREAD_PRIORITY);
    return (thread != NULL);
}
*/
...
#endif
    if(stackSize!= EHS_THREAD_USE_DEFAULT_STACK_SIZE) // set custom stack size
    {
        //printf("custom thread stack size = %d\n", stackSize);
