...
#ifdef INX_TEST_INX_TEMP
    printf("DEBUG-V: EhsTgtTimer_tick: target=%3d\ttime=%10d\n",EhsTgtTimerExpiryTime, EHS_CURRENT_TIME);
#endif
...
//        if (EHS_TARGET_TIME_IS_EARLIER(EhsTgtTimerExpiryTime,EHS_CURRENT_TIME))
        ...
        // be OK!
#ifdef INX_TEST_INX_TEMP
        printf("DEBUG-V: EhsTgtTimer_tick: tick!\n");
#endif
        EhsTimer_tick();
        }
    }
...
void EhsSleep(EhsTickType tSleepTime)
{
    ...
    vTaskDelay(tSleepTime <= EHS_TICKS_PER_EHS_FREERTOS_TICK ? 1 : tSleepTime / EHS_TICKS_PER_EHS_FREERTOS_TICK); 
}

void EhsSleepUs(ehs_uint32 tSleepTime)
{
    EhsSleep(EhsTgtTimer_usToTick(tSleepTime));
    gTimeTest = EhsTgtTimer_usToTick(tSleepTime);
}
