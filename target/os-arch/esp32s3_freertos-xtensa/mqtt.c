...
//todo2024 - do we need a hal later or just include the FB heaer for these protoypes
//unsigned int EhsMQTTPublishWritePoll(char *topic, char *payload, unsigned int *qos);

bool EhsMQTTSubscribeWritePoll(char *buffer, bool *subscribe, uint8_t *qos);
bool EhsMQTTSubscribeEvent(char *topic, char* payload, int payloadSize);
ehs_bool EhsMqttPublishCallback(char *topic, char *payload, int qos);
...
#ifdef INX_STARTED_TO_INTEGRATE
...
void EhsMQTTConnectPoll(ehs_bool *connect, char **host, ehs_uint16 *pPort, ehs_uint16 *pTLS, char **clientid, char **username, char **password, char **clientCertFileName, char **clientKeyFileName, char **rootCAFileName);
...
ehs_bool EhsMqttPublishCallback(char *topic, char *payload, int qos)
{
    ...
    if (MQTT_state == MQTT_STATE_CONNECTED)
    {
        ...
        if (payload[0] != 0)
        {
            esp_mqtt_client_publish(client, topic, payload, 0, qos, 0); // Retain flag is 0. Auto detect payload length
            return EHS_TRUE;
        }
        else {
