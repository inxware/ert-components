...
ehs_bool EhsTF_rmdir(ehs_char* szPath)
{
    ...
    if (EhsTF_exists(szPath) >= 1)   //@todo this will remove a file also - is this what we want?
    {
        ...
        ESP_LOGI(TAG, "EhsTF_rmdir ret=%d",ret);
    }
    else
    {
        ret=-1;
    }
    ESP_LOGI(TAG, "EhsTF_rmdir ret=%d szPath=%s",ret,szPath);
    return (ret == 0);
}
...
/* todo  this is supposed to close a bunvh file or a directory? */
...
void listdir(char *dir)
{
    ...
    while((entry = readdir(dp)) != NULL) 
    {
        ...
        if(stat(temp, &statbuf) == 0)
        {
            ...
            {
                // Print file name
                ESP_LOGI(TAG, "%s/%s", dir, entry->d_name);
            }
        }
    }
...
ehs_bool EhsTgtFilesystem_Init(void)
{
    ...
    esp_vfs_littlefs_conf_t conf_appdata = {
        .base_path = "/appdata",
        .partition_label = "appdata",
        .format_if_mount_failed = true,
        .dont_mount = false,
    };
    ...
    if (ret != ESP_OK)
    {
            if (ret == ESP_FAIL)
            {
                    ESP_LOGE(TAG, "Failed to mount or format filesystem \"storage\"");
            }
            ...
            {
                    ESP_LOGE(TAG, "Failed to initialize LittleFS \"storage\" (%s)", esp_err_to_name(ret));
            }
            return EHS_FALSE;
    }

    ret = esp_vfs_littlefs_register(&conf_appdata);
    if (ret != ESP_OK)
    {
            if (ret == ESP_FAIL)
            {
                    ESP_LOGE(TAG, "Failed to mount or format filesystem \"appdata\"");
            }
            else if (ret == ESP_ERR_NOT_FOUND)
            {
                    ESP_LOGE(TAG, "Failed to find LittleFS partition \"appdata\"");
            }
            else
            {
                    ESP_LOGE(TAG, "Failed to initialize LittleFS \"appdata\" (%s)", esp_err_to_name(ret));
            }
            return EHS_FALSE;
    }
    ...
    if (ret != ESP_OK)
    {
            ESP_LOGE(TAG, "Failed to get LittleFS partition \"storage\" information (%s)", esp_err_to_name(ret));
    }
    else
    {
            ESP_LOGI(TAG, "Partition \"storage\" size: total: %d, used: %d", total, used);
    }
    EhsHMetaSetInstPath(base_path);
    listdir(base_path);

    ret = esp_littlefs_info(conf_appdata.partition_label, &total, &used);
    if (ret != ESP_OK)
    {
            ESP_LOGE(TAG, "Failed to get LittleFS partition \"appdata\" information (%s)", esp_err_to_name(ret));
    }
    else
    {
            ESP_LOGI(TAG, "Partition \"appdata\" size: total: %d, used: %d", total, used);
    }
    listdir("/appdata");
