...
/**
 * @file target_uart.c  (QNX Neutrino)
 * @brief QNX Neutrino serial UART HAL implementation.
 *
 * Opens /dev/ser<N> (or any path set via TgtUart_SetDevicePath) using
 * standard POSIX termios.  A background pthread is spawned per port in
 * TgtUart_Start(); it blocks on select() with a 100 ms timeout and calls
 * the registered callback whenever bytes arrive.
 *
 * QNX differences vs Linux HAL:
 *   - Default device paths: /dev/ser1, /dev/ser2, ... (QNX convention).
 *   - pthreads are in libc on QNX SDP 7.1+ — no separate -lpthread.
 *   - All termios, select(), read(), write() calls are identical POSIX.
 *
 * Receive model
 * -------------
 * The receive thread blocks in select() with 100 ms timeout.  It wakes
 * immediately when bytes arrive; 100 ms is only the worst-case empty-loop
 * interval.  Latency is OS scheduling jitter (typically <1 ms on QNX).
 */
...
/* -----------------------------------------------------------------------
 * Device path table — QNX convention /dev/ser<N>
 * ----------------------------------------------------------------------- */

#define QNX_DEV_PATH_LEN 64
static char s_dev_paths[UART_COUNT][QNX_DEV_PATH_LEN] = {
    QNX_UART0_DEV,
#if UART_COUNT > 1
    QNX_UART1_DEV,
#endif
...
int TgtUart_SetDevicePath(int UART_num, const char *path)
{
    ...
    snprintf(s_dev_paths[UART_num], QNX_DEV_PATH_LEN, "%s", path);
    return TgtUART_OK;
}
                  ...
                  int flow_control)
{
    ...
    if (fd < 0)
    {
        EHSH_LOG_ERROR("QNX UART%d: open %s failed: %s",
    ...
    {
        EHSH_LOG_ERROR("QNX UART%d: tcgetattr failed: %s", UART_num, strerror(errno));
        close(fd);
    ...
    {
        EHSH_LOG_ERROR("QNX UART%d: tcsetattr failed: %s", UART_num, strerror(errno));
        close(fd);
        return TgtUART_CONFIG_ERROR;
    }
    ...
    {
        EHSH_LOG_ERROR("QNX UART%d: pthread_create failed: %s",
    ...
    EHSH_LOG_INFO("QNX UART%d: started on %s at %d baud",
...
int TgtUart_RS485Setup(int UART_num)
{
    ...
    /* RS-485 TX-enable on QNX requires a board-specific resource manager
     * or external transceiver with automatic TX-enable. */
    EHSH_LOG_INFO("QNX UART%d: RS-485 auto TX-enable not configured; "
                  "use a transceiver with automatic TX-enable", UART_num);
    return TgtUART_OK;
}
...
int TgtUart_Stop(int UART_num)
{
    ...
    s_cb[UART_num]         = NULL;
    s_configured[UART_num] = EHS_FALSE;
    EHSH_LOG_INFO("QNX UART%d: stopped", UART_num);
    return TgtUART_OK;
}
...
int TgtUart_Send(int UART_num, char *payload, unsigned int length)
{
    ...
    if (written < 0)
    {
        EHSH_LOG_ERROR("QNX UART%d: write failed: %s", UART_num, strerror(errno));
        return TgtUART_ERROR;
    }
...
int TgtUART_RecvPoll(int UART_num, char *payload, unsigned int length)
{
    ...
    if (n < 0)
    {
        EHSH_LOG_ERROR("QNX UART%d: read failed: %s", UART_num, strerror(errno));
        return TgtUART_ERROR;
    }
