...
/**
 * @file target_uart.h  (QNX Neutrino)
 * @brief QNX Neutrino serial UART HAL declarations.
 *
 * Uses standard POSIX termios serial I/O — identical API to the Linux
 * implementation.  A background pthread is spawned per port in
 * TgtUart_Start(); it blocks in select() with a 100 ms timeout and
 * invokes the registered callback via TgtUART_Intr_register() when
 * bytes arrive.
 *
 * QNX differences vs Linux HAL:
 *   - Default device paths use QNX convention: /dev/ser1, /dev/ser2 …
 *   - pthreads are in libc on QNX SDP 7.1+ — no separate -lpthread.
 *   - The UART_CALLBACK_FUNCTIONS initializer list is otherwise identical.
 *
 * Device path defaults (override at compile time via DEFS, e.g.):
 *   DEFS += QNX_UART0_DEV=\"/dev/serusb1\"
 *
 * Or override at runtime (before TgtUart_Start) using:
 *   TgtUart_SetDevicePath(UART_num, "/dev/ser2");
 */
...
/* -----------------------------------------------------------------------
 * Device path defaults — QNX serial device naming: /dev/ser<N>
 * ----------------------------------------------------------------------- */

#ifndef QNX_UART0_DEV
#define QNX_UART0_DEV "/dev/ser1"
#endif

#ifndef QNX_UART1_DEV
#define QNX_UART1_DEV "/dev/ser2"
#endif

#ifndef QNX_UART2_DEV
#define QNX_UART2_DEV "/dev/ser3"
#endif

#ifndef QNX_UART3_DEV
#define QNX_UART3_DEV "/dev/ser4"
#endif
...
/* -----------------------------------------------------------------------
 * Pin macros (not applicable to QNX serial devices; provided for API
 * compatibility only — values are ignored by TgtUart_Start)
 * ----------------------------------------------------------------------- */
...
#define UART_BAUDRATE_MAX 4000000
...
/** Set device path at runtime (before TgtUart_Start). E.g. "/dev/ser3". */
int TgtUart_SetDevicePath(int UART_num, const char *path);

/** Windows COM port — no-op on QNX */
int TgtUart_SetComPort(int UART_num, int com_port_number);

/* Yield briefly in spin loops to avoid burning the CPU */
#define TgtUART_WDT_feed() sched_yield()
