...
/**
 * @file target_uart.h  (linux)
 * @brief Generic Linux POSIX serial UART HAL declarations.
 *
 * Uses standard termios serial I/O.  A background receive thread is
 * spawned per port in TgtUart_Start(); it blocks on read() with a
 * 100 ms timeout and invokes the callback registered via
 * TgtUART_Intr_register() whenever bytes arrive.
 *
 * Device path selection:
 *   Port 0 → LINUX_UART0_DEV  (default: "/dev/ttyUSB0")
 *   Port 1 → LINUX_UART1_DEV  (default: "/dev/ttyUSB1")
 *
 * Override at compile time via CFLAGS / DEFS, e.g.:
 *   DEFS += LINUX_UART0_DEV=\"/dev/ttyAMA0\"
 *
 * Or override at runtime (before TgtUart_Start) using:
 *   TgtUart_SetDevicePath(UART_num, "/dev/ttyS0");
 */
...
#ifndef LINUX_UART0_DEV
#define LINUX_UART0_DEV "/dev/ttyUSB0"
#endif

#ifndef LINUX_UART1_DEV
#define LINUX_UART1_DEV "/dev/ttyUSB1"
#endif

#ifndef LINUX_UART2_DEV
#define LINUX_UART2_DEV "/dev/ttyUSB2"
#endif

#ifndef LINUX_UART3_DEV
#define LINUX_UART3_DEV "/dev/ttyUSB3"
#endif
...
/* -----------------------------------------------------------------------
 * Pin macros (not applicable to Linux serial devices — provided for
 * API compatibility only; values are ignored by TgtUart_Start)
 * ----------------------------------------------------------------------- */
...
/** Set device path at runtime (before TgtUart_Start). E.g. "/dev/ttyAMA0". */
int TgtUart_SetDevicePath(int UART_num, const char *path);

/** Windows-specific no-op on Linux */
