...
ehs_bool EhsTV_init(EhsTVClass* pViewport)
{
	...
	while (!go) {g_usleep(10000);} //Wait for gtk init to have completed (except the GTK Main part).
	//go2 = 1; //flag back to EhsL_graphicsThreadFunc (gtk thread) that we have started

	//printf("X:gone!\n"); // Tell the OSD window we're done here
...
#if GTK_MAJOR_VERSION == 2
		printf("!!!!!!!!!!! no RGBA colormap available for this target \n");
...
#define EHS_NEED_A_TIMER_FOR_GRAPHICS
	...
	go=1;
	while (1) {gtk_main_iteration_do(0);g_usleep(50000);} //this seems less crashy than the gtk_main() blocker!
...
//	printf("expose rect left[%d],top[%d],width[%d],height[%d]\n",exposeRect.nLeft,exposeRect.nTop,exposeRect.nWidth,exposeRect.nHeight);


	if ( (pEvent->area.height == 0 ) || (pEvent->area.width == 0) ) goto skip;
	//printf("w=%d, h=%d\n",pEvent->area.height,pEvent->area.width );
#ifdef EHS_MINGW
	...
	{
//		printf("intersect rect left[%d],top[%d],width[%d],height[%d]\n",intersectViewClip.nLeft,intersectViewClip.nTop,intersectViewClip.nWidth,intersectViewClip.nHeight);
//		printf("\n");
...
//		printf("\n\nExpose alpha=[%d]\n", viewColour.sComp.nAlpha);
//		printf("Expose exposeRect.nLeft=[%d],exposeRect.nTop=[%d],exposeRect.nWidth=[%d],exposeRect.nHeight=[%d]\n", exposeRect.nLeft,exposeRect.nTop,exposeRect.nWidth,exposeRect.nHeight);
...
gboolean EhsL_event_key_release(GtkWidget* pWidget, GdkEventKey* pEvent)
{
	...
	EhsGUIKbHit(pEvent->keyval); // call all the prescribing function blocks
		if(pEvent->keyval == 27) //'ESCAPE' releases the keyboard from this window
		{
			...
			printf ("GDK Ungrabbing the Keyboard\n");
		}
		else if(pEvent->keyval == 103) //'g' grabs it - but of course it is already !!!
		{
			...
			printf ("GDK Grabbing the Keyboard\n");
		}
		*/
...
//	printf("GTK:EhsL_event_key_press - ENTER - keycode=%d\n",pEvent->keyval);
	EhsGUIKbHit(pEvent->keyval); // call all the prescribing function blocks
...
//	printf("EhsL_event_button_press x=[%d],y=[%d]\n",x,y);
}
...
//	printf("EhsL_event_button_release x=[%d],y=[%d]\n",x,y);
}
...
//	printf("EhsL_event_motion_notify x=[%d],y=[%d]\n",x,y);
}
...
void EhsTV_updateRect(EhsTVClass* pViewport, ehs_sint32 nX, ehs_sint32 nY, ehs_sint32 nWidth, ehs_sint32 nHeight)
{
	...
	//printf("DirtyRectangle after nLeft: %i nTop: %i nWidth: %i nHeight: %i \n", globalRect.nLeft, globalRect.nTop, globalRect.nWidth, globalRect.nHeight);
...
#error This is unsafe
			...
			case EHS_GRAPHICS_COLOUR_A1:
				if (nPaletteSize >= 2u)
				{
					...
					{
						/* alloc failed, reset pSurface */
					//printf("ERROR: Couldn't get the bitmapan");
						bFailed = EHS_TRUE;
					}
				}
				else
...
ehs_bool EhsTV_getScreenSize(ehs_sint32 *nScreenWidth, ehs_sint32 *nScreenHeight) {
	   ...
	   *nScreenHeight = gdk_screen_get_height(screen);
	   //printf("XXX Screen Width = %d, height =%d\n",*nScreenWidth,*nScreenHeight);
}
