...
/* Notes*/
/* This seems to support normal MQTT function blocks and also OTa firmware and SODL updates (with some kind of built in topics)
   todo2024 -see how the OTA support might correspond/conflict to the more generic OTA update method in DevmanMon.c 
*/
 ...
 * todo2024 Move this to the targetos_init.c
 */
...
static char gMqttPayload[INX_MQTT_MAX_PAYLOAD_SIZE]; //todo2024 is the MQTT payload size fixed for all MCU type targets?
...
/* Devman specific bits that we will keep for now */
static int32_t gMqttInpubId=-1; /* inx / Devman speocific - ID of incoming data after something is published*/
...
//HRDONLY:static volatile bool gGenerateCommissioningReport=false;
//HRDONLY:volatile bool gGenerateDiagnosticReport=false;
//HRDONLY:static volatile bool gUserTemperatureAlarmTriggeredPreviousValue=false;
//HRDONLY:extern inxPID_t gPIDs[CONFIG_PID_CONTROLLERS_NUM];
...
#ifdef INX_MQTT_SUPPORT_LEGACY_OTA
/*****************************************************************************************************/
/* todo2024 OTA stuff to remove */
...
#ifdef INX_MQTT_SUPPORT_PROVISIONING
...
/* presumably this is just to set the client ID as the prefix in the JSON */
...
/* @brief This has some Devman Specific features
  This function Writes MQTT data into global gInxIncomingTopic buffer 
  Does this check for overwrites? 
  todo2024 - we probably don't want any of this for eRT?
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    tempString[len]='\0';
    //todo2024 should this next line be after the if so it only happens when the last fragment is received?
    
    if(flags & MQTT_DATA_FLAG_LAST)
    {
        
        EhsMQTTSubscribeEvent(gInxIncomingTopic,tempString,len);
        ...
        /* ********************************************************************************
        The following is all to support legacy HRD Devman stuff and can probably be removed,
        unless it might be sueful to define what standard HRD devman APIs is...
        ************************************************************************************/
#ifdef INX_MQTT_SUPPORT_HARDWIRED_CONTROLLER_DATA        
        uint32_t result;
...
/* @brief this decodes includes some Devman-specific MQTT topics such as setting the time
...
*/
static void mqtt_incoming_devman_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    ...
    if(status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        mqtt_set_inpub_callback(client, mqtt_incoming_devman_publish_cb, mqtt_incoming_data_cb, arg);
    }
    else
    {
...
/* This is the main period loop 
   todo2024 this should be moved into the existing network loop. 
*/
void* EhsMqttClientLoop(void* args __attribute__((unused)))
//void mqttMainLoop()  - old name
{
    ehs_bool connect=0;
    ehs_char* host=NULL;
    ehs_char* clientid=NULL;
    ehs_char* username=NULL;
    ehs_char* password=NULL;
    ehs_char* clientCertFileName=NULL;
    ehs_char* clientKeyFileName=NULL;
    ehs_char* rootCAFileName=NULL;
    ehs_uint16 port=0;
...
/* Early NXP didn't have any kind of file system, so these are used, which are not super aligned with other EHS file abstraction, but is part of the small.
these are like an mmap function!*/
    ...
    case MQTT_STATE_CONNECTED:
        ...
        else
        {
            ...
            ehs_bool subscribe=false;
            uint8_t qos=0;
            bool needProcessing=EhsMQTTSubscribeWritePoll(gTopicNameTemporary,&subscribe, (ehs_uint8*)&qos);
            if(needProcessing==false)
            {
                needProcessing=EhsMQTTPublishWritePoll(gTopicNameTemporary,gMqttPayload, (ehs_uint8*)&qos);
                if(needProcessing==false)
                {
    ...
    case MQTT_STATE_DO_DISCONNECT:
        ...
        inxLedNetworkModeSet(INX_LED_NETWORK_MODE_LOCAL); //HRD hardware only
        break;
    default:
        break;
    }
