...
ehs_bool gUsingDevmanMon = EHS_FALSE;
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_UNSUBSCRIBED:
        EHS_ESP32_MQTT_LOG("MQTT_EVENT_UNSUBSCRIBED, msg_id=%d\n", event->msg_id);
        break;
    ...
    case MQTT_EVENT_ERROR:
        ...
        {
            EHS_ESP32_MQTT_LOG("Last error code reported from esp-tls: 0x%x\n", event->error_handle->esp_tls_last_esp_err);
            ...
            // EHS_ESP32_MQTT_LOG("Last tls stack error number: 0x%x", event->error_handle->esp_tls_stack_err);
            // EHS_ESP32_MQTT_LOG("Last captured errno : %d (%s)", event->error_handle->esp_transport_sock_errno,
            //        strerror(event->error_handle->esp_transport_sock_errno));
        }
        else if (event->error_handle->error_type == MQTT_ERROR_TYPE_CONNECTION_REFUSED)
        {
            EHS_ESP32_MQTT_LOG("Connection refused error: 0x%x\n", event->error_handle->connect_return_code);
            EhsSprintf(mqtt_error_msg_buffer, "connection refused error : 0x%x", event->error_handle->connect_return_code);
            EhsMQTTReportError(mqtt_error_msg_buffer);
        }
        else
        {
            EHS_ESP32_MQTT_LOG("Unknown error type: 0x%x\n", event->error_handle->error_type);
...
ehs_bool readAppFileIntoString(ehs_bool isDevmanMon, const char *filename, char **output)
{
    ehs_FILE *tempFile;
    ...
    if(isDevmanMon == EHS_TRUE){
        // look for the certificates in the devman directory
    ...
    {
        EHS_ESP32_MQTT_LOG("readAppFileIntoString: Error reading file\n");
        EhsFclose(tempFile);
        return EHS_FALSE;
    }
    else
...
void* EhsMqttClientLoop(void* args)
{
    ...
    case MQTT_STATE_CONNECTED:
        ...
        if (gSubscribe)
        {
            ...
            //EHS_ESP32_MQTT_LOG("Subscribe defined. Topic: %s, QoS: %d\n", gTopicNameTemporary, gQoS);
    ...
    case MQTT_STATE_DO_DISCONNECT:
        EHS_ESP32_MQTT_LOG("MQTT_STATE_DO_DISCONNECT\n");
    ...
    case MQTT_STATE_DISCONNECTED:
        EHS_ESP32_MQTT_LOG("MQTT_STATE_DISCONNECTED\n");
        EhsMQTTConnectPoll(&connect, &host, &port, &gUseTLS, &clientid, &username, &password, &clientCertFileName, &clientKeyFileName, &rootCAFileName);
        if (connect)
        {
        ...
        else
        {
            ...
            EHS_ESP32_MQTT_LOG("topic: %p, QoS: %p\n", topic_list, gQoS_list);
            if (topic_list != NULL)
            {
                free(topic_list);
                topic_list = NULL;
    ...
    case MQTT_STATE_DISCONNECTED_NEW:
        EHS_ESP32_MQTT_LOG("MQTT_STATE_DISCONNECTED_NEW\n");
        gSubsribeAgain = EHS_FALSE;
        if (topic_list != NULL){
            for (i = 0; i < topic_count; i++)
            {
