...
/*
Test Code for the ADC hardware
...
static dac_oneshot_handle_t dac_channel_handles[EHS_TARGET_DAC_NUMBER] = { NULL };
...
ehs_bool EhsTDacConfigure(ehs_uint8 channel, ehs_sint32 max, ehs_sint32 sample_rate)
{
    ...
    dac_oneshot_config_t chan_cfg = {
        .chan_id = channel == 0 ? DAC_CHAN_0 : DAC_CHAN_1
    };
    EPS_ERROR_CHECK(dac_oneshot_new_channel(&chan_cfg, &(dac_channel_handles[channel])));
...
ehs_bool EhsTDacSet(ehs_uint8 channel, ehs_sint32 value)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(
        dac_oneshot_output_voltage(dac_channel_handles[channel],
        ((value > dac_channel_max[channel] ? dac_channel_max[channel] : value < 0 ? 0 : value) << 8) / dac_channel_max[channel]
        )
    );
    return EHS_TRUE;
}
...
ehs_bool EhsTDacDestroy(ehs_uint8 channel)
{
    if (channel >= EHS_TARGET_DAC_NUMBER) return EHS_FALSE;
    if (dac_channel_handles[channel] == NULL) return EHS_FALSE;
    ESP_ERROR_CHECK_WITHOUT_ABORT(dac_oneshot_del_channel(dac_channel_handles[channel]));
