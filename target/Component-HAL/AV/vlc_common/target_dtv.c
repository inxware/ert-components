...
ehs_bool Ehs_TVInstallstopCallBack(EhsFunctionInstanceDataType* pFIdata,
	...
	{
		//printf("Adding EhsTDPlayEndCallback->fpRunFunc=%X, EhsTDPlayEndCallback->pFuncInst=%X",EhsTDPlayEndCallback->fpRunFunc, EhsTDPlayEndCallback->pFuncInst);
		...
		//printf("removig EhsTDPlayEndCallback->fpRunFunc=%X, EhsTDPlayEndCallback->pFuncInst=%X",EhsTDPlayEndCallback->fpRunFunc, EhsTDPlayEndCallback->pFuncInst);
...
//printf("\nEhsVlcMediaInstanceStatusPlaying\n");
...
void * end_function_callback_vlc_eventid_remover(
	...
	/*
	 ...
	 //printf("libvlc_media_player_get_state %d, (libvlc_Playing=%d libvlc_Paused=%d\n",playing,libvlc_Playing,libvlc_Paused);
...
ehs_bool EhsTDPlayback_loadFile(EhsFunctionInstanceDataType* pFIdata,
	...
	if (pPlayback->mediaFormat != EHS_UNKNOWN_MEDIA_TYPE) {
		//printf("XX-EhsTDPlayback_loadFile: %s pPVR=%x, pPlayback=%x\n",szFilename,pPVR,pPlayback );
		if (pPlayback->pVLCInstance) {
			...
			if (!EhsStrncmp("http://", pPlayback->LoadedFilename, 7) || EhsTF_exists(pPlayback->LoadedFilename)) { //@todo otherwise fire a can't play  event
				...
				if (pPlayback->nMedia = libvlc_media_new_path(
					...
					if (pPlayback->nMediaPlayer != NULL) {
					 ...
					 if(!RegisterClassEx(&wc)) {printf("COULDN't register wc\n"); return 0;}
					 else printf("Registered");
						 */
						//if(!Register(HIn)) { printf("COULDN't register the hinst\n"); return 0;
						...
						//printf("\n\nDoing the LOAD thing - assigning the hwn\n\n");
						//EhsTDPlayback_setWindow(pPVR); // set the window to the coords
						//if (pPlayback->window) {
						//ShowWindow(pPlayback->window,SW_SHOW);
						//UpdateWindow(pPlayback->window); SetFocus(pPlayback->window);
						//	printf("Already have a HWND from a previous session?=%x\n",pPlayback->window,libvlc_media_player_get_hwnd(pPlayback->nMediaPlayer));
						...
						//printf("\nXXXXXXDQSet = %x Retrieved HWND=%x\n",pPlayback->window,libvlc_media_player_get_hwnd(pPlayback->nMediaPlayer));
						//}
						//else {
						//	printf("\nEhsTDPlayback_setWindow no window handle\n");
						//}
...
float EhsTV_getPosition(libvlc_media_player_t* pMP) {
	...
	if ((playing == libvlc_Playing) || playing == libvlc_Paused) {
		if (pMP) {
			...
			//printf("Position  to 0\n");
		}
	} else {
		//printf("(XXXXXXXXXXXXX6)\n");
	}
	return position;
}
/*
 if (!libvlc_exception_raised(EhsTV_excp)) {
 printf("LibVLC Exception: Leaving in 'Loaded' state!!%d\n", libvlc_exception_get_message(&EhsTV_excp));
...
void EhsTV_setTime(libvlc_media_player_t* pMP, libvlc_time_t nTime) {
	...
	libvlc_state_t playing = libvlc_media_player_get_state(pMP);
	printf("Setting time - Playing = %d (playing == %d || paused == %d\n",
			playing, libvlc_Playing, libvlc_Paused);
	if ((playing == libvlc_Playing) || playing == libvlc_Paused) {
		if (pMP) {
...
int EhsTV_pause(libvlc_media_player_t* pMP) { //return 0 when moving from paused to unpaused and for moving paused from playing.
	...
	libvlc_state_t playing = libvlc_media_player_get_state(pMP);
	//printf ("Pause-state=%d, playing=%d:paused=%d\n",playing,libvlc_Playing,libvlc_Paused);
	if ((playing == libvlc_Playing) || playing == libvlc_Paused) {
...
void EhsTV_stop(libvlc_media_player_t* pMP) {
	...
	if (pMP) {
		...
		if ((playing == libvlc_Playing) || (playing == libvlc_Paused ) ) {//libvlc_Stopped ) && (playing != libvlc_Ended)) { //perhaps we need to add libvlc ==ended here too?
			...
			//printf("libvlc_media_player_get_state(pMP, &EhsTV_excp)=%d\n",	playing);
...
#ifndef EHS_NO_MEDIAPLAYER_VIDEO
if (pPlayback->mediaFormat == EHS_VIDEO) {
if ((pPlayback->xPlaybackState == EHS_FB_STATE_LOADED) || (pPlayback->xPlaybackState == 	EHS_FB_STATE_PLAYING) || (pPlayback->xPlaybackState == EHS_FB_STATE_PAUSED)) { // ignore if we have no immediate prospect of playing anything.
#ifdef EHS_GUI_SUPPORT
	//printf("EhsTDPlayback_setWindow: Enter");
	//only do window stuff if size is above the minimum of 21 (based on vlc1.1.2 testing) - PBB 20100904
	...
	if (( pPvrPlay->nWidth_adjusted > 21) && ( pPvrPlay->nHeight_adjusted > 21)) {
		...
		//printf("Setting Window - pPlayback->window=%x\n",pPlayback->window);
		if (pPlayback->window == (Window) NULL) {
			...
			if (pPlayback->pGTKWindow->window) {
				pPlayback->window
						= GDK_WINDOW_XID(gtk_widget_get_window(pPlayback->pGTKWindow));
printf("Sett the XID for the video widget to %x\n",pPlayback->window);
		...
		if( pPlayback->window== (Window)NULL) {
			...
			if(!RegisterClassEx(&wc)) {
				printf("Couldn't register win32 stuff\n");
			}
		...
		{
			printf("pPlayback->pGTKWindow already initialised -Memory Leek? ");
		}
		...
		if( pPlayback->window==NULL) { //
			...
			if (pPlayback->window) {
				...
				//}
				//printf("SetWindowPos(window_found=windowX=%x,windowY=%x,windowW=%x,windowH=%x\n;",windowX,windowY,windowW,windowH);
				if (pPvrPlay->video_on_top) {
				...
				//printf("Video Trying to Loosing FOCUS!!!!!!!!!!!!!!!!!!!!!\n ");
		//}
		//SendMessage(window_found, WM_KILLFOCUS, NULL, NULL);
				/*
				printf("windowX=%d,windowY=%d,windowW=%d,windowH=%d\n",windowX,windowY,windowW,windowH);
	...
	//printf("EhsTDPlayback_setWindow: Exit");


#endif /* EHS_GUI_SUPPORT */
}
else {
	//printf("Not moving a window because we don't want one at the moment\n");
}
