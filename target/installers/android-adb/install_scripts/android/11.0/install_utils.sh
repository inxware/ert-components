...
adbconnect(){
	...
	if [ -z "$IP" ]; then
		...
		echo "Connecting adb to $IP with port = $UPORT"
		$ADB connect "$IP${UPORT}"
		sleep 1
		$ADB root
		sleep 1
		$ADB connect "$IP${UPORT}"
...
check_android_version(){
	...
	if [ "$VERSION" = "11" ]; then
		...
		echo "Target device android version ($VERSION) doesn't match configuration (11)."
		exit 1
	fi
}
