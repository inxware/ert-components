...
#define TEST_TRIM(expected,operation,actual) EhsStrcpy(szTestString, actual); if (0 != EhsStrcmp(expected,operation(szTestString))) {szRet = szErr; EhsSprintf(szErr,"%s <> %s()%s",#expected,#operation,#actual);}
	...
	if (NULL != EhsStrTrimL(NULL)) { szRet = "expected Null from EhsStrTrimL(NULL)"; }
	if (NULL != EhsStrTrimR(NULL)) { szRet = "expected Null from EhsStrTrimR(NULL)"; }
	if (NULL != EhsStrTrimLR(NULL)) { szRet = "expected Null from EhsStrTrimLR(NULL)"; }
